...
<head>
    ...
    <title>Edit Pengguna – SMAN 1 Tanjungpinang</title>
    ...
    <div class="admin-layout">
        ...
        <main class="admin-main">
            ...
            <div class="admin-header-bar">
                <div>
                    <h1 style="font-size: 1.5rem; color: var(--primary-dark);">Sunting Data Pengguna</h1>
                    <p style="font-size: 0.85rem; color: var(--text-muted);">Ubah informasi personal, hak akses, atau sandi pengguna.</p>
                </div>
                    ...
                    <form action="{{ route('admin.users.update', $user->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            ...
                            @if($user->id === Session::get('admin_id'))
                                ...
                                <span style="font-size: 0.75rem; color: var(--text-muted); display: block; margin-top: 0.25rem;">Anda tidak dapat mengubah hak akses akun Anda sendiri demi menjaga integritas sistem.</span>
                            @endif
                            @error('role')
                                <span style="font-size: 0.8rem; color: #ef4444; font-weight: 500;">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="form-group" style="margin-bottom: 2.5rem;">
                            <label for="password">Kata Sandi Baru (Password) (Kosongkan jika tidak diganti)</label>
                            <input type="password" id="password" name="password" class="form-control" placeholder="••••••••">
                            <span style="font-size: 0.75rem; color: var(--text-muted); display: block; margin-top: 0.25rem;">Isi kolom ini jika hanya ingin mengatur ulang kata sandi pengguna.</span>
                            @error('password')
                                <span style="font-size: 0.8rem; color: #ef4444; font-weight: 500;">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <button type="submit" class="btn-accent" style="width: 100%; justify-content: center; font-size: 1rem; padding: 0.85rem;">
                            Simpan Perubahan Akun <i class="fa-solid fa-circle-check"></i>
                        </button>
                    </form>
                </div>
