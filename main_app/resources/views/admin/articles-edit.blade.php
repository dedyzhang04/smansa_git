...
<head>
    ...
    <title>Edit Berita SMANSA – SMAN 1 Tanjungpinang</title>
    ...
    <div class="admin-layout">
        ...
        <main class="admin-main">
            ...
            <div class="admin-header-bar">
                <div>
                    <h1 style="font-size: 1.5rem; color: var(--primary-dark);">Edit Artikel / Berita</h1>
                    <p style="font-size: 0.85rem; color: var(--text-muted);">Sunting liputan kegiatan, pengumuman, atau artikel pembelajaran sekolah.</p>
                </div>
                ...
                <div class="admin-card" style="max-width: 800px; margin: 0 auto;">
                    <form action="{{ route('admin.articles.update', $article->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" id="title" name="title" class="form-control" placeholder="Masukkan judul berita..." value="{{ old('title', $article->title) }}" required>
                            @error('title')
                        ...
                        <div class="admin-grid-2">
                            <div class="form-group">
                                ...
                                <select id="category" name="category" class="form-control" required style="-webkit-appearance: none; appearance: none; background-image: url('data:image/svg+xml;utf8,<svg fill=%22%23044a27%22 height=%2224%22 viewBox=%220 0 24 24%22 width=%2224%22 xmlns=%22http://www.w3.org/2000/svg%22><path d=%22M7 10l5 5 5-5z%22/><path d=%22M0 0h24v24H0z%22 fill=%22none%22/></svg>'); background-repeat: no-repeat; background-position: right 1rem center; background-size: 1.25rem;">
                                    <option value="utama" {{ old('category', $article->category) === 'utama' ? 'selected' : '' }}>Utama / Berita Utama</option>
                                    <option value="umum" {{ old('category', $article->category) === 'umum' ? 'selected' : '' }}>Umum / Berita Umum</option>
                                    <option value="pendidikan" {{ old('category', $article->category) === 'pendidikan' ? 'selected' : '' }}>Pendidikan</option>
                                    <option value="iptek" {{ old('category', $article->category) === 'iptek' ? 'selected' : '' }}>IPTEK (Sains/Teknologi)</option>
                                </select>
                                @error('category')
                                    <span style="font-size: 0.8rem; color: #ef4444; font-weight: 500;">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="form-group">
                                ...
                                <input type="text" id="author" name="author" class="form-control" placeholder="Nama penulis" value="{{ old('author', $article->author) }}" required>
                                @error('author')
                                    <span style="font-size: 0.8rem; color: #ef4444; font-weight: 500;">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <!-- Image inputs -->
                        <div class="form-group" style="border: 1px dashed rgba(11, 99, 197, 0.2); padding: 1.5rem; border-radius: 12px; margin-bottom: 2rem; background-color: rgba(11, 99, 197, 0.01);">
                            ...
                            @if($article->image)
                                <div style="margin-bottom: 1rem;">
                                    <p style="font-size: 0.8rem; font-weight: 700; color: var(--primary-dark); margin-bottom: 0.5rem;">Gambar Saat Ini:</p>
                                    <img src="{{ $article->image }}" alt="Sampul artikel" style="max-height: 150px; border-radius: 8px; border: 1px solid rgba(0,0,0,0.1);">
                                </div>
                            @endif

                            <div class="form-group">
                                <label for="image_file">Pilihan 1: Unggah File Gambar Baru (Mengganti)</label>
                            ...
                            <div class="form-group" style="margin-bottom: 0;">
                                <label for="image_url">Pilihan 2: Gunakan URL Gambar Baru</label>
                                <input type="url" id="image_url" name="image_url" class="form-control" placeholder="https://example.com/gambar.jpg" value="{{ old('image_url', Str::startsWith($article->image, 'http') && !Str::contains($article->image, asset('')) ? $article->image : '') }}">
                                @error('image_url')
                                    <span style="font-size: 0.8rem; color: #ef4444; font-weight: 500;">{{ $message }}</span>
                                @enderror
                            </div>
                        ...
                        <div class="form-group" style="display: flex; align-items: center; gap: 1rem; margin-top: 1rem; margin-bottom: 2.5rem; background: rgba(11, 99, 197, 0.03); padding: 1.25rem; border-radius: 10px; border: 1px solid rgba(11, 99, 197, 0.08);">
                            <input type="checkbox" id="is_featured" name="is_featured" value="1" {{ old('is_featured', $article->is_featured) ? 'checked' : '' }} style="width: 20px; height: 20px; cursor: pointer; accent-color: var(--primary-color);">
                            <div>
                        ...
                        <button type="submit" class="btn-accent" style="width: 100%; justify-content: center; font-size: 1rem; padding: 0.85rem;">
                            Simpan Perubahan Artikel <i class="fa-solid fa-circle-check"></i>
                        </button>
                    </form>
                </div>
